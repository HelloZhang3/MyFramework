package com.zyd.myframework.widget.wheel;
 ...
 * @author JiangPing
 */
public class WheelViewAddress extends View {
    /**
     * 控件宽度
     */
    ...
    public WheelViewAddress(Context context, AttributeSet attrs,
                     int defStyle) {
    ...
    public WheelViewAddress(Context context) {
        super(context);
        initData();
    }
     ...
     * @param currentText
     */
    public void setDefault(String currentText){
    	if (currentText.equals("街道")) {
			return;
		}
    	
    	for (ItemObject itemObject: itemList) {
			String text = itemObject.itemText;
			
			if (text.equals(currentText)) {
				float move = itemObject.moveToSelected();
		        defaultMove((int) move);
			}
		}
    }
